...
const sheetVariants = cva(
  'fixed z-[50] gap-space-4 bg-surface p-space-6 shadow-overlay transition ease-in-out data-[state=closed]:duration-200 data-[state=open]:duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
...
>(({ side = 'right', className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-space-4 top-space-4 rounded-sm opacity-70 ring-offset-surface transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2 disabled:pointer-events-none">
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn('text-h2 text-default font-semibold', className)}
    {...props}
  />
));
