...
This is the prompt to feed an AI assistant at the start of every coding session in this repository. Paste it (or attach the `nestjs-ai-guardrails/` folder) before asking for code.
...
1. `nestjs-ai-guardrails/00-README.md` — hard rules and the definition of done.
2. `nestjs-ai-guardrails/01-ARCHITECTURE.md` — layered architecture, folder layout, dependency rules.
3. `nestjs-ai-guardrails/02-CONVENTIONS.md` — naming, file structure, imports, **code comment formatting**, banned patterns.
4. `nestjs-ai-guardrails/03-NESTJS.md` — modules, DI, pipes, guards, interceptors, lifecycle.
5. `nestjs-ai-guardrails/04-DATA.md` — Prisma patterns, repositories, transactions, tenant scoping.
6. `nestjs-ai-guardrails/05-API.md` — DTOs, validation, error responses, REST and GraphQL contract.
7. `nestjs-ai-guardrails/06-EVENTS.md` — EventEmitter, BullMQ, naming, idempotency.
8. `nestjs-ai-guardrails/07-TESTING.md` — unit, e2e, factories, coverage targets.
9. `nestjs-ai-guardrails/08-SECURITY.md` — auth, transport security, tenant isolation, validation, secrets.
10. `nestjs-ai-guardrails/09-RBAC.md` — role hierarchy, resource scoping, policy-based authorization via CASL, permission matrix, audit log.
...
6. Follow the naming rules in `02-CONVENTIONS.md` exactly. File names, identifiers, suffixes.
