...
The board does not try to be a planner. Schedules are built in the schedule view (`08-schedule-view.md`). The board is the operational present.
...
`VERIFIED` is intentionally hidden from this board. Once a coordinator verifies a job, it rolls off after a tunable delay (default 30 minutes, set per tenant in `11-settings.md`). The rationale is the board shows in-flight work, not history. Verified jobs live in the jobs index (`05-data-display.md`).
...
A card is a single focusable element. `Enter` or `Space` opens the drawer. `Tab` moves to the next focusable inside the card (worker pill, then menu trigger if visible). Arrow keys do not move between cards; this is documented in `13-accessibility-and-motion.md` as a known affordance the keyboard user gets through column navigation instead.

## Realtime updates

The board subscribes to the tenant's job event stream over WebSocket. The transport is documented in `09-webhooks-and-events.md`. The UI rules below apply regardless of source.
...
Keyboard equivalent uses `Space` to lift, arrow keys to move between columns, `Space` again to drop, and `Escape` to cancel. Documented further in `13-accessibility-and-motion.md`.
...
- The tunable delay for VERIFIED roll-off has no settings UI yet; see `11-settings.md` for where it will live.
