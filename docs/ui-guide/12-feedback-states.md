...
| Worker history | No completed jobs yet. | Your history will show up here. | none |
...
|---|---|---|
| Dispatch board card move | Optimistic, rollback with toast on failure | chapter 06 |
| Worker mobile check-in | Queued offline, replay on reconnect | chapter 07 |
| Settings save | Pessimistic, no optimistic update | chapter 11 |
| Member role change | Optimistic, rollback with toast on failure | chapter 10 |
...
When `navigator.onLine === false`, the topbar (chapter 02) gains a band along its bottom edge in `--color-warn-fade` with `--color-warn` text reading `You are offline. Changes will sync when you reconnect.` The band is 28px tall on desktop and 32px on mobile.
...
| Worker view write (check-in, check-out, status update) | Queued locally, replayed on reconnect. See chapter 07. |
...
- The offline indicator on the worker mobile shell may need a stronger affordance than a band; chapter 07 will decide.
