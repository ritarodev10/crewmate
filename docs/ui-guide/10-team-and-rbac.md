...
The surface where a `tenant_admin` manages who is on the team, what role each member holds, and what slice of the tenant they can reach. The model behind this UI is authoritative in `nestjs-ai-guardrails/09-RBAC.md`. This chapter is the visual and behavioral contract for the screens that expose that model.
...
A `coordinator` reads the same page to know who their peers are. A `worker` never reaches it; the route returns 403 and the sidebar item is hidden. A `super_admin` sees the same page across any tenant they act in, with one extra column (Tenant) and the Impersonate action on every non-self row.
...
| `/settings/team/roles` | Built-in roles read-only, custom roles management (v0.2) |
| `/settings/audit` | Audit log, linked from the bottom of the members page |

The Settings sidebar nests both `team` and `team/roles` under the Team item. See `02-layout-and-navigation.md` for the nested sidebar pattern.
...
| Impersonate | `tenant_admin`, `super_admin` (never on self, never on `super_admin` rows) |
...
A user can hold more than one `RoleGrant`, per `nestjs-ai-guardrails/09-RBAC.md`. In the table, the primary pill renders the highest-privileged role using the privilege order above. A small suffix `+1 more` (or `+2 more`) renders in `text-small text-muted` to the right of the pill with `space-1` gap. The suffix is not clickable; the full list of grants lives in the Drawer.
...
| Expiry | When the grant has an `expiresAt` (v0.2), shown in `text-small`. v0.1 grants have no expiry |
...
| Impersonate | `primary` | `tenant_admin`, `super_admin`, never on self | Opens the impersonation `ConfirmDialog` |

The footer wraps on narrow viewports; buttons stack below `sm` with `Impersonate` on top and `Revoke access` on the bottom. A `coordinator` Drawer has no footer; the body is read-only.
...
The `/settings/team/roles` sub-page. v0.1 ships read-only; the New role and edit flows are deferred to v0.2 per `nestjs-ai-guardrails/09-RBAC.md`.

### Layout

| Region | Content |
|---|---|
| Page header | Title `Roles` in `text-h1`, trailing `New role` Button `variant="primary"` (disabled in v0.1 with a tooltip `Custom roles arrive in v0.2`) |
| Built-in roles | Section titled `Built-in roles` in `text-h2`, a `Card` per built-in role |
| Custom roles | Section titled `Custom roles` in `text-h2`, a `Card` per custom role. Empty in v0.1 |
...
The summary uses `text-small` with `text-mono` for the verb column. A `LinkButton` at the bottom reads `See transition matrix` and links to `nestjs-ai-guardrails/09-RBAC.md`.

### Custom role card (v0.2 shape)
...
### New role dialog (v0.2 shape, documented now)

Opens from the `New role` button when it lands in v0.2. The Dialog uses a wider `lg` variant of 720px to fit the permission matrix.
...
Inapplicable cells render an em-dash glyph in `text-muted` and are not clickable. The action vocabulary follows the `Action` type from `nestjs-ai-guardrails/09-RBAC.md`. A footer note reads `Custom roles cannot grant access broader than your own. Some cells are disabled because of your current permissions.` Disabled cells render at 50% opacity with a `Tooltip` naming the missing actor permission.
...
Per `nestjs-ai-guardrails/09-RBAC.md`, every grant change, Revoke, Resend, and Impersonate start writes to the permission audit log. The audit page surfaces those rows.

## Impersonation start

The active-impersonation banner, the chrome, and the Stop impersonating flow are owned by `03-auth-flows.md`. This section covers only the start handoff from the team surface.

A `tenant_admin` or `super_admin` clicks `Impersonate` in the Drawer footer or selects `Impersonate` from the row action menu. Either path opens a `ConfirmDialog`.

Dialog copy.

```text
View as <name>?
You will see CrewMate as <name>. Every action you take is recorded against you, with their permissions. You can stop at any time.
```

The footer carries `Cancel` `variant="secondary"` and `Start session` `variant="primary"`. The primary is not `variant="danger"`, per `03-auth-flows.md`; impersonation is sensitive, not destructive. On confirm the client swaps to the act-as token, full-reloads to `/today` for a worker target or `/dispatch` for a coordinator target, and the impersonation banner appears across the chrome from `03-auth-flows.md`.

The Impersonate menu item is hidden in three cases.

| Case | Reason |
|---|---|
| The actor is the target | Self-impersonation makes no sense |
| The target is a `super_admin` and the actor is `tenant_admin` | Cannot impersonate above your own role |
| The actor lacks the `read User` ability on the target | They cannot see the user at all |
...
| Impersonate | Yes | Yes, except `super_admin` targets and self | n/a | n/a |
| Resend invitation | Yes | Yes | n/a | n/a |
| Revoke invitation | Yes | Yes | n/a | n/a |
| Create custom role (v0.2) | Yes | Yes | n/a | n/a |
| Edit custom role (v0.2) | Yes | Yes | n/a | n/a |
...
- Revoke access requires typing the name and uses `variant="danger"`. Impersonate hands off to the banner from `03-auth-flows.md`.
- `/settings/team/roles` renders built-in cards in v0.1; `View audit log` routes to `/settings/audit?category=authorization`.
...
- Custom roles are deferred to v0.2. The `/settings/team/roles` page renders built-in cards in v0.1; the New role button is disabled with the documented tooltip. The Dialog and matrix shape are documented here so the v0.2 work is layout-only.
- Time-bound grants (`expiresAt` on `RoleGrant`) are deferred to v0.2. The Drawer Grants section reserves the Expiry slot and renders nothing for v0.1 grants.
- Field-level masking on `Worker` (hourly rate, contact details) is deferred to v0.2. The Notification preferences section ships read-only in v0.1; v0.2 widens this with masking aware of the actor's grants.
