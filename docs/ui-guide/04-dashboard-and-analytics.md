...
The Overview page sits inside the standard app shell from `02-layout-and-navigation.md`. The page header carries the title `Overview` in `text-h1`, and two trailing controls aligned to the right. The trailing slot holds the date range picker, then the export button, in that order.
