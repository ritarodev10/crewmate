...
Every screen a user sees before they have a session, plus the screens that change a session once they have one. Login, invitation acceptance, password reset, two-factor authentication, impersonation, sign-out, and session expiry.
...
All auth pages outside the in-app overlays (2FA enrollment, impersonation banner) use the same two-panel split. Left panel sets brand, right panel does the work.
...
The email contains a one-time link with a 30 minute expiry. UI surface for the email lives in chapter 11 transactional templates. The link routes to `/auth/reset/confirm?token=...`.
...
A `tenant_admin` or `super_admin` can act as another user inside the operator. Per `09-RBAC.md`, the act-as token records both `actorId` and `effectiveUserId` on every action, and the token has a short TTL.

### Start UI

The start UI lives in `/settings/team`. Each row in the member list shows a `DropdownMenu` `IconButton` at the row's right edge. For users the actor is allowed to impersonate, the menu includes an item "View as <name>". For users the actor is not allowed to impersonate (their peers or themselves), the item is hidden.

Selecting "View as <name>" opens a `ConfirmDialog`:

```
Start viewing as <name>?
You'll see CrewMate exactly as <name> sees it. Every action you take will be
logged against your account and theirs. Sessions end after 30 minutes or when
you stop.
[ Cancel ]   [ View as <name> ]
```

The primary button uses `variant="primary"`, not `variant="danger"`. Impersonation is sensitive, not destructive.

On confirm, the client swaps to the act-as token and full-reloads to `/today` or `/dispatch` depending on the target user's role.

### Active impersonation banner

A `Banner` renders at the very top of every page, above the topbar.

| Property | Spec |
|---|---|
| Position | `position: sticky`, `top: 0`, `z-index: 60` (one above the topbar) |
| Height | 40px |
| Fill | `bg-[--color-danger]` |
| Text | `text-on-brand` (white), `text-body-strong` |
| Layout | Centered, "You are viewing as <name>" left, "Stop impersonating" `Button variant="secondary"` on the right with white border 1px on `bg-[--color-danger]` |
| Dismiss | None. The banner is sticky and cannot be hidden. |
| Reduced-motion | Static, no slide-in |

The topbar shifts down by 40px while the banner is active. Page content's first sticky element starts at `top: 40px + topbar height`.

A 60s before token expiry, the banner adds a trailing countdown in `text-small`, "Ends in 0:57". At expiry the client transparently restores the original session and the banner disappears with a neutral `Toast`, "Stopped viewing as <name>."

### Stop impersonating

Clicking "Stop impersonating" calls `/v1/auth/impersonation/stop`, swaps the token back to the original, and full-reloads to `/dashboard`. A `Toast` confirms.

### Impersonation error states

| Cause | Treatment |
|---|---|
| Target user is in a different tenant | Item not shown in menu |
| Target user is `super_admin` and actor is `tenant_admin` | Item not shown in menu |
| Act-as token request fails | `Toast variant="error"`, "Couldn't start impersonation. Try again." |
| Act-as token expires mid-action | The next request returns 401, the client restores the original session and shows a banner on the landing page, "Impersonation ended. You are back as yourself." |
...
### Edge case, refresh during impersonation

If the access token expires while the user is impersonating, the refresh attempt uses the act-as refresh cookie. On failure, the client restores the original session (if its refresh is still valid) and lands on `/dashboard`. If neither is valid, the user is signed out fully and lands on `/login?reason=expired`.
...
| Impersonation start | `Toast error` | n/a | n/a | n/a |
...
- The impersonation banner uses `role="status"` and `aria-live="polite"`. It is not an alert because it persists.
...
- **Account deletion lives in `/settings/profile`,** documented in chapter 11, not here.
- **No rendered impersonation banner exists yet.** `[NEEDS: docs/images/ui/impersonation-banner.png]`.
