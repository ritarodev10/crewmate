...
The schedule view is the coordinator's weekly read of every worker at once. The dispatch board (chapter 14) answers "what is happening right now"; this screen answers "what does next week look like, and is anyone overloaded". A coordinator opens this view on Sunday evening, scrolls the seven columns, and looks for empty cells that should be full, full cells that should be empty, and overlaps that need a phone call.
...
The page uses the standard app shell from chapter 03. The page body holds the schedule grid and nothing else.
...
The page-level topbar (chapter 03) holds the schedule controls. Reading left to right.
...
Opens the job `Drawer` from chapter 14. Same drawer, same content, same close behavior. The schedule view does not have a custom job detail surface.
...
- Job creation. That is the dispatch board's job in chapter 14.
- The job drawer body. That lives in chapter 12.
