...
Webhook deliveries, team members, and audit logs are tables. The dispatch board is a kanban (see `14-dispatch-board.md`). The mobile worker today view is a card list (see `15-worker-mobile.md`).
...
See `10-accessibility-and-motion.md` for the full a11y patterns.
