...
The reference visual lives at `docs/images/ui/webhook-log.png`. The event taxonomy this UI surfaces is defined in `./shared/02-events.md`.
...
The table at `/webhooks`. See chapter 12 for the `Table`, `Pagination`, and `StatusPill` base patterns this screen reuses. This chapter only documents the deltas.
...
The base spec is in chapter 12. The columns specific to this surface are listed below.
...
Row click selects the row, which opens the detail panel. The selected row uses the standard table selected-row treatment from chapter 12 (left border 2px `bg-brand`, fill `bg-brand-soft`).

### Pagination

Cursor-based, since the log is append-only and can grow unbounded. The footer renders the current range ("1 to 25 of 248") on the left and prev/next controls on the right. See chapter 12 for the `Pagination` component.
...
The full canonical list is in `./shared/02-events.md`. The UI groups events by domain, in this order. Each row in the subscriptions form and each option in the event filter combobox comes from this table.
...
The worker mobile shell at `/app` does not surface this section at all. No nav entry, no deep link path. A direct URL navigation returns the 403 full-page error state from chapter 08.
...
- The DLQ admin tool referenced in `./shared/02-events.md` is not surfaced in the UI. For v0.1 the assumption is that the deliveries log plus the "Retry failed" action is enough. The DLQ has its own backing table but no screen yet. `[NEEDS: DLQ admin screen or explicit decision to keep it engineer-only]`
...
- Webhook health metrics over time (delivery rate, p95 latency per endpoint) are not in this chapter. The analytics surface in chapter 13 does not cover them either. `[NEEDS: delivery health metrics, owner undecided]`
