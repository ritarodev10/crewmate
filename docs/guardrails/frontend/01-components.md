...
Optional leading icon. The pill accepts an `icon` prop that renders a Lucide glyph at `icon-xs` (14px) in `currentColor`, immediately to the left of the label with a `space-1` gap. Use it when the pill is the primary read on a busy surface (e.g. the screen-title status indicator in `03-layout-and-navigation.md`).

```tsx
<StatusPill variant="success" icon={<CheckCircle2 />}>Live</StatusPill>
<StatusPill variant="progress" icon={<Eye />}>Preview</StatusPill>
<StatusPill variant="neutral" icon={<Clock />}>Planned</StatusPill>
```
...
- Loading and error states come from the hook (`isPending`, `isError`, `error`) and feed into the components in this catalog. See `08-feedback-states.md` for the patterns.
...
- Page-level layout. See `03-layout-and-navigation.md`.
...
- A11y deep dive. See `10-accessibility-and-motion.md`.
